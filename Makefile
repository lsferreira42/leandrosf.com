...
SOURCE_DIRS     := $(shell gfind $(SOURCE_DIR) -mindepth 1 -type d)
SOURCE_MARKDOWN := $(shell gfind $(SOURCE_DIR) -type f -name '*.md' -and ! -name $(BLOG_LIST_FILE))
SOURCE_STATIC   := $(shell gfind $(SOURCE_DIR) -type f -regextype posix-extended -iregex '$(STATIC_REGEX)')
BLOG_POSTS      := $(shell gfind $(SOURCE_DIR)/$(BLOG_DIR) -type f -name '*.md' -and ! -name $(BLOG_LIST_FILE) -and ! -path $(SOURCE_DIR)/$(BLOG_DIR)/index.md)
...
$(OUTPUT_DIR)/index.html: $(SOURCE_DIR)/index.md $(SOURCE_DIR)/$(BLOG_LIST_FILE) $(TEMPLATE)
	gsed $$'/$(BLOG_LIST_REPLACE)/{r $(SOURCE_DIR)/$(BLOG_LIST_FILE)\nd}' $< | $(PANDOC) --template=$(TEMPLATE) --output=$@
...
$(OUTPUT_DIR)/$(BLOG_DIR)/index.html: $(SOURCE_DIR)/$(BLOG_DIR)/index.md $(SOURCE_DIR)/$(BLOG_DIR)/$(BLOG_LIST_FILE) $(TEMPLATE)
	gsed $$'/$(BLOG_LIST_REPLACE)/{r $(SOURCE_DIR)/$(BLOG_DIR)/$(BLOG_LIST_FILE)\nd}' $< | $(PANDOC) --template=$(TEMPLATE) --output=$@
